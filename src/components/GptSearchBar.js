...
const GptSearchBar = () => {
  ...
  const handleGptSearchClick = async () => {
    ...
    //       message: {
    //         role: "assistant",
    //         content: "Don, Superbad, Bridesmaids, The Hangover, Step Brothers",
    //       },
    //     },
    //   ],
    // };
    if (!gptResults.choices) return "Failed to fetch results from gpt apis";
    const gptMovies = gptResults.choices?.[0]?.message?.content?.split(",");
    //search each movie in TMDB api's
